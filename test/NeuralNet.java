...
public class NeuralNet 
{
	...
	public static void testTraining( NeuralNet n1, double[] trainingData )
	{
		int epochs = 20;
		int miniBatchSize = trainingData.length / epochs;
		int dataCount = 0;
		double n = 0.9;
		...
		for( int i = 0; i < epochs; i++ )
		{
			dataCount = miniBatchSize * i;
			for( int j = 0; j < miniBatchSize; j++ )
			{
				
				for( int k = 0; k < n1.layers[0].length; k++ )
				{
					n1.layers[0][k] = trainingData[k];
				}
				//insertInputData( n1, trainingData.get( j + dataCount ) ); // Insert Data
				...
				// 2. Output Error
				errorArray[ j ][ n1.layers.length - 2 ] = getOutputError( n1.layers, z, y );
				
				// 3. Backpropagate the Error
				backpropagate( n1.weights, z, errorArray[ j ] );
			}
			
			originalCost = originalCost / ( 2 * miniBatchSize) ;
			System.out.println( "#" + i + "|Cost:|" + originalCost + "|" );

//			System.out.println( "Mini Batch #" + i + " accomplished." );
//			for( int g = 0; g < errorArray[0][ n1.layers.length-2 ].length; g++ )
//			{
//				System.out.println( g + "Error Array Sample:" + errorArray[0][ n1.layers.length-2 ][g] );
//			}
	...
	public static void main( String[] args ) throws IOException
	{
		...
		ArrayList<List<String>> trainingData = CSVReader.ReadData( "/home/misha/Documents/JavaProjects/NeuralNetwork/data/mnist_train.csv" );
		System.out.println("COMPLETED - Reading *TRAINING DATA*" );
		
		System.out.println("Reading *TEST DATA*\n..." );
		ArrayList<List<String>> testData = CSVReader.ReadData( "/home/misha/Documents/JavaProjects/NeuralNetwork/data/mnist_test.csv" );
